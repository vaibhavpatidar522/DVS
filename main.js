...
let contractAddress = "0x6C6aD41eFe9B8024e19226cddEA893244Ee3223e";
let contractAbi =[
  {
    "inputs": [
      {
...
async function loadStateCityDropdown() {
  ...
  try {
      const [stateNames, stateIndices, cityNames, cityIndices] = await contractInstance.getStateCityInfo();
...
async function updateCityDropdown() {
  ...
  try {
      const [_, __, cityNames, cityIndices] = await contractInstance.getStateCityInfo();
      const citySelect = document.getElementById("citySelect");
...
async function updateStateGraph() {
  ...
  const stateIndex = document.getElementById("stateSelect").value;
  try {
    const stateVotes = await contractInstance.getCandidateVotesInCity(stateIndex, 0);
    ...
    const statePartyTable = document.getElementById('statePartyTable').getElementsByTagName('tbody')[0];
    ...
    const stateWinnerParty = partyNames[partyVotes.indexOf(Math.max(...partyVotes))];
    document.getElementById('stateWinner').innerHTML = `Winner Party from Selected State: ${stateWinnerParty}`;
...
async function updateCityGraph() {
  ...
  try {
    ...
    const cityCandidateTable = document.getElementById('cityCandidateTable').getElementsByTagName('tbody')[0];
    ...
    const cityWinnerCandidate = candidateNames[candidateVotes.indexOf(Math.max(...candidateVotes))];
    document.getElementById('cityWinner').innerHTML = `Winner Candidate from Selected City: ${cityWinnerCandidate}`;
...
const getAllCandidatesForVote = async () => {
  if (WALLET_CONNECTED) {
      ...
      const userAadharNumber = document.getElementById("aadharNumberInput").value.trim();
      if (!userAadharNumber) {
      ...
      try {
          const [user, candidates] = await contractInstance.getCandidatesForVoter(userAadharNumber);
...
const addVote = async (aadharNumber, stateIndex, cityIndex, candidateIndex) => {
  ...
  try {
      const tx = await contractInstance.vote(aadharNumber, stateIndex, cityIndex, candidateIndex);
      await tx.wait();
...
const getVotingStatus = async () => {
  ...
  try {
      ...
      const { days, hours, minutes, seconds } = convertSecondsToDHMS(remainingTime);
      document.getElementById("status").innerHTML = `Voting Status: ${status}, Time Remaining: ${days} days, ${hours} hours, ${minutes} minutes, and ${seconds} seconds`;
  } catch (error) {
      console.error(error);
...
async function updateOverallWinnerGraph() {
  ...
  try {
    const [stateNames, stateIndices] = await contractInstance.getStateCityInfo();
    const overallPartyVotes = {};

    for (let i = 0; i < stateIndices.length; i++) {
        const stateVotes = await contractInstance.getCandidateVotesInCity(stateIndices[i], 0);
        const stateParties = stateVotes[1];
        const stateVoteCounts = stateVotes[2].map(votes => parseInt(votes));
    ...
    const overallPartyTable = document.getElementById('overallPartyTable').getElementsByTagName('tbody')[0];
    ...
    const overallWinnerParty = partyNames[partyVotes.indexOf(Math.max(...partyVotes))];
    document.getElementById('overallWinner').innerHTML = `Overall Winner Party: ${overallWinnerParty}`;
